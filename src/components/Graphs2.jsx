...
function getExpense(data){
  ...
  for(var i = 0;i<data.length;i++){
    if(data[i].ExpenseClass === "Grocery"){
      datanew.Grocery+=parseInt(data[i].amount)
    }
    else if(data[i].ExpenseClass === "Tech Purchase"){
      datanew.TechPurchase+=parseInt(data[i].amount)
    }
    else if(data[i].ExpenseClass === "Travel"){
      datanew.Travel+=parseInt(data[i].amount)
    }
    else if(data[i].ExpenseClass === "Food"){
      datanew.Food+=parseInt(data[i].amount)
    }
  }
  console.log(datanew)
  return ([
